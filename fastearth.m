function [uv,ue,H,xs,ys]=fastearth(Nin,tf,dtyear,Z,type,FLAG)
...
%     2 eta |grad| du/dt + rho g u + D grad^4 u = sigma_zz.
...
%     held.
%
% [UV,UE,HH,XX,YY]=FASTEARTH(N,TF,DTYEAR,Z,'...',FLAG)  
%     Same as above except:
%         FLAG = 1: Add a low viscosity channel.  See equation (15) in
%                   Bueler et al (2006)
%         FLAG = 2: No graphing.
%         FLAG = 3: Low viscosity channel and no graphing.
%
...
% See also GEFORCONV, VISCDISC, TESTFAST, BEDSHEET.
% ELB 1/12/06; 10/19/06

if nargin<5, Cflag=false; type='disc'; end
graphflag=true;
visclayers=1;
if nargin >= 6
    if FLAG >= 2, graphflag=false; end
    if ((FLAG == 1) || (FLAG == 3)), visclayers=2; end
end
...
if (visclayers==2) %See equation (15) in Bueler et al (2006)
    % thin, low viscosity channel modifications
    etatil = 0.04;  % ratio of thin channel viscosity to mantle viscosity
    Tc = 75e3;   % thin channel thickness
    kap = cchalf;
    Cthin = cosh(Tc*kap);  Sthin = sinh(Tc*kap);
    CS = Cthin.*Sthin;  Csqr = Cthin.*Cthin;  Ssqr = Sthin.*Sthin;
    Rthin = 2*etatil*CS + (1-etatil^2)*Tc^2*kap.*kap + ...
        etatil^2*Ssqr + Csqr;
    Rthin = Rthin ./ ...
        ( (etatil+1/etatil)*CS + Tc*(etatil-1/etatil)*kap +Ssqr+Csqr );
    cchalf = Rthin .* cchalf;
end
